...
function Clock() {
  ...
  const start = () => {
    ...
    const timerId = Number(setInterval(() => setDate(new Date()), 1000));
setTimerId(timerId);
  ...
  const stringTime = `${date.getHours().toString().padStart(2, "0")}:${date
    .getMinutes()
    .toString()
    .padStart(2, "0")}:${date.getSeconds().toString().padStart(2, "0")}` || (
    <br />
  ); // часы24:минуты:секунды (01:02:03)/(23:02:03)/(24:00:00)/(00:00:01) // пишут студенты
  const stringDate = `${date.getDay().toString().padStart(2, "0")}.${date
